...
const DoctorHistory = ({ auth, appointments }) => {
    return (
        <AuthenticatedLayout
            user={auth.user}
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        {appointments.map((appointment) => (
                            <div key={appointment.id} className="p-6 bg-white border-b border-gray-200">
                                <p className="text-gray-900 font-semibold">Consulta com paciente {appointment.patient.name} - <span className="font-normal">{appointment.date_time}</span></p>
                                ...
                                <ul className="list-disc pl-5 mt-2">
                                    {appointment.prescriptions.map((prescription) => (
                                        <li key={prescription.id} className="text-gray-700">
                                            {prescription.drug?.name || 'Fármaco Desconhecido'} - Dosagem: <span className="font-semibold">{prescription.dosage}</span>
